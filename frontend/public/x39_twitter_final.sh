#!/usr/bin/env bash
# ============================================================================
#  X-39MATRIX :: TWITTER THREAD FINAL · OPTIMIZADO POST-FINNEY
#  - 13 tweets (12 + cierre con CTA verify)
#  - Hook killer + verify command pinned
#  - Tributo Hal Finney en tweet 11 (cuadruple anchor)
#  - 100% data real verificada en mainnet
#
#  USO:
#    bash <(wget -qO- https://estado-protocolo.preview.emergentagent.com/x39_twitter_final.sh)
# ============================================================================
set -uo pipefail
G="\033[1;32m"; B="\033[1;34m"; N="\033[0m"

REPO="${HOME}/x39matrix-web"
OUT="${REPO}/outreach/01_twitter_thread.md"
[ -d "${REPO}/outreach" ] || mkdir -p "${REPO}/outreach"
...
git add outreach/01_twitter_thread.md
if ! git diff --cached --quiet; then
  git commit -m "outreach(twitter): thread final 13-tweets · tributo Hal Finney en 11/13 · CTA verify pinned" || true
...
echo
echo -e "${B}═══════════════════════════════════════════════════${N}"
echo -e "${G} Twitter thread FINAL listo:${N}"
echo "    https://x39matrix.org/outreach/01_twitter_thread.md"
echo "    o local: $OUT"
echo
echo " Para postear:"
echo "   cat $OUT | less"
echo "   → copiar tweet 1/13, pegar en x.com/compose/tweet"
echo "   → esperar 90s, reply con 2/13, asi sucesivamente"
echo
echo " Hora optima: 16:00 CEST (= 14:00 UTC) cualquier dia entre semana"
echo -e "${B}═══════════════════════════════════════════════════${N}"
