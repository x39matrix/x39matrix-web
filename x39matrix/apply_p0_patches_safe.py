#!/usr/bin/env python3
"""X-39MATRIX :: arn4r P0 patches :: 7 endpoints :: bash-safe variant"""
import hashlib, sys
from pathlib import Path

EXPECTED_PRE_HASH = "d45374d3b632e560410174775cbc143a4312d66749a8ec46edad7aaa7a0df78d"

# Mismo guard que el resto del canister pero usando .all(p != caller)
# en vez de .any(p == caller) negado, equivalente por De Morgan.
GUARD_TRAP = """    let caller = ic_cdk::api::caller().to_text();
    if SOVEREIGN_PRINCIPALS.iter().all(|p| *p != caller.as_str()) {
...
# Post-process: substituye 'format(' por 'format!(' una vez metido en disco
# (evitamos el bang en el texto fuente que bash come).
def restore_bang(s):
    return s.replace("format(", "format!(")

PATCHES = [(o, restore_bang(n), name) for (o, n, name) in PATCHES]
